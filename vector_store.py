...
class CodeBERTIndexer:
 ...
 # Normalizare L2 a embeddings-urilor pentru cosine similarity prin dot product
 norms = np.linalg.norm(all_embeddings, axis=1, keepdims=True) + 1e-9
 all_embeddings_normalized = all_embeddings / norms
 
 # Creăm indexul FAISS cu Inner Product (echivalent cosine pe vectori normalizati)
 self.index = faiss.IndexFlatIP(dimension)
 self.index.add(all_embeddings_normalized)
 ...
 # 1. Căutare Semantică (FAISS — cosine similarity prin IndexFlatIP)
 query_vector = self.get_embeddings([query]).astype('float32')
 # Normalizăm query-ul L2 pentru a asigura cosine similarity corect
 q_norm = np.linalg.norm(query_vector) + 1e-9
 query_vector_normalized = query_vector / q_norm
 # Căutăm în FAISS pe toate documentele disponibile ca să le combinăm
 similarities, indices = self.index.search(query_vector_normalized, len(self.chunks))
 
 semantic_scores = np.zeros(len(self.chunks))
 for i, idx in enumerate(indices[0]):
 if idx != -1 and idx < len(self.chunks):
 # Inner product pe vectori normalizati = cosine similarity in [-1, 1]
 # Mapăm la [0, 1]: (cos_sim + 1) / 2
 raw_cos = float(similarities[0][i])
 semantic_scores[idx] = (raw_cos + 1.0) / 2.0
 ...
 s_sem = float(semantic_scores[idx])
 s_lex = float(lexical_scores[idx])
 ...
 chunk["semantic_score"] = s_sem
 chunk["lexical_score"] = s_lex
