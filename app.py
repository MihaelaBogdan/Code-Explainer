...
"""
 ...
 "react": {
 "title": " React.js (Dezvoltare Frontend modernă)",
...
def _generate_dynamic_fallback(topic):
 ...
 lines.append("#### Piloni Principali de Aplicare:")
...
if not st.session_state.project_processed:
 ...
 with st.expander(" Panou de Control Căutare (Parametri Hibrid & Filtrare)", expanded=False):
...
• Lexicală (TF-IDF): {chunk.get('lexical_score', 0.0):.1%}"""
 ...
 rank_badge = " Code Architect"
 badge_color = "#38bdf8"
 badge_glow = "rgba(56,189,248,0.4)"
 elif best_score >= 50:
 rank_badge = " Code Apprentice"
 badge_color = "#fbbf24"
 ...
 badge = '<span style="background:rgba(239,68,68,0.15);color:#f87171;border:1px solid rgba(239,68,68,0.3);padding:3px 8px;border-radius:12px;font-size:0.8em;font-weight:bold;"> Revizuiește</span>'
 ...
 with col_btn2:
 st.write("")
 if st.button(" Șterge Istoric", use_container_width=True, type="secondary", key="clear_history_btn"):
 st.session_state.quiz_history = []
 st.rerun()
 ...
 st.markdown(" **Logica extrasă din ambele fragmente:**")
...
<br><span style='color:#64748b;font-size:0.82em;'> Dacă nu e apelată din exterior (API/test/UI), poate fi ștearsă în siguranță.</span>
 ...
 concepts = {
 " Baze de date & Stocare": "database query sql sqlite postgresql select insert storage tables",
 ...
 " Paralelism & Procese": "subprocess shell command execution multithreading async concurrent thread",
 " Utilitare Text & Fișiere": "regex string parsing utility zip directory extraction write load split"
 }
 ...
 with tab7:
 st.markdown("## Analizor de Impact Semantic (AI) — Schimbări în Cod")
 ...
 pass
  ...
  # Formula academica multi-factor:
  # - 60% pondere dependente AST fizice directe (cel mai important indicator)
  # - 30% pondere corelații semantice CodeBERT (propagare conceptuala)
  # - 10% pondere complexitate relativa (linii de cod)
  ast_weight = min(1.0, len(direct_callers) / 4.0) * 60
  
  # Ponderea semantica: diferentiem clar intre corelatie inalta si medie
  sem_factor = (len(high_sem_correlations) * 1.0 + len(medium_sem_correlations) * 0.35)
  semantic_weight = min(1.0, sem_factor / 5.0) * 30
  
  n_lines = max(0, el - sl)
  lines_weight = min(1.0, n_lines / 100.0) * 10
  
  impact_score = int(ast_weight + semantic_weight + lines_weight)
  impact_score = max(5, min(impact_score, 100)) if (direct_callers or high_sem_correlations or medium_sem_correlations) else 0
 ...
 st.markdown("### Harta Propagării Impactului")
